  ...
  export default {
    ...
    methods: {
      ...
      initEditor () { 
        ...
        this.editor.txt.html(this.activityDetail)  // 讲获取的内容展示在文本编辑器中
      },

    },
    watch: {
      activityDetail(newVal, oldVal) {
        if (newVal != oldVal) {
          this.editor.txt.html(this.activityDetail)
        }
      },
      editorContent(newVal, oldVal) {
        if (newVal != oldVal) {
          this.setActivityDetail(this.editorContent)
        }
      }
    }
  }
