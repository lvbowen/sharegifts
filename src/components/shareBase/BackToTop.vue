<template>
  <transition :name="transitionName">
    <div v-show="show" :style="customStyle" class="back-to-ceiling" @click="backToTop">
...
<script>
  export default {
    ...
    computed: {
      show() {
        return this.scTop > this.visibilityHeight
      }
    },
    methods: {
      backToTop() {
        const start = this.scTop
        let i = 0
        this.interval = setInterval(() => {
          const next = Math.floor(this.easeInOutQuad(10 * i, start, -start, 500))
          if (next <= this.backPosition) {
            this.$emit('scrollTo', this.backPosition - 50)
            // window.scrollTo(0, this.backPosition)
            clearInterval(this.interval)
          } else {
            this.$emit('scrollTo', next - 50)
